...
knitr::include_graphics("/Users/kristensyme/Dropbox/FastingLLMPub/World Map of 56 Cultures.png")
```
...
Following procedures outlined in @dubourg2024, we refined prompts through repeated trial and error to assess GPT-4.0’s annotation accuracy. This approach uses Parameter-Efficient Fine-Tuning (PEFT), which allows models to be adapted to new tasks by adjusting only a small number of parameters, such as prompts rather than retraining the entire model. @liu2022 demonstrated that optimized prompt tuning can achieve results comparable to full fine-tuning, evidencing the efficiency and effectiveness of prompt-based approaches for adapting large language models to specialized tasks like ethnographic annotation. 
